...
const ViewParticularFormByID = () => {
    ...
    const getFormsForFormId = async () => {
        ...
        try {
            const getdata = await api.get(`userform/getparticularformnewalldetails?formid=${formid}`)

            if (getdata.data.status) {
                toast.success(getdata.data.message)
                setdata(getdata.data.data);
                setuser(getdata.data.main_user)
                setdesignated(getdata.data.designated_user)
                settracking(getdata.data.tracking_data)
                setfinance(getdata.data.tracking_data.financial_year)
            }
            else {
                ...
                <div class="flow-root mb-5 border p-4 rounded-lg shadow-lg">
                    <dl class="-my-3 divide-y divide-gray-100 text-base">
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            ...
                            <dd class="text-gray-900 sm:col-span-2">{user?.user_first_name}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Employee ID</dt>
                            <dd class="text-gray-900 sm:col-span-2">{user?.emp_id}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Department</dt>
                            <dd class="text-gray-900 sm:col-span-2">{user?.department_name}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Designation</dt>
                            <dd class="text-gray-900 sm:col-span-2">{user?.designation}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Date of Joining</dt>
                            <dd class="text-gray-900 sm:col-span-2">{user?.doj ? moment(user.doj).format("DD-MM-YYYY") : "-"}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Financial</dt>
                            <dd class="text-gray-900 sm:col-span-2">{finance}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Designated Person</dt>
                            <dd class="text-gray-900 sm:col-span-2">{designated?.user_first_name}</dd>
                        </div>
                        <div class="grid grid-cols-1 gap-1 py-1.5 sm:grid-cols-4 sm:gap-4">
                            <dt class="font-medium text-blue-600">Designated Person ID</dt>
                            <dd class="text-gray-900 sm:col-span-2">{designated?.emp_id}</dd>
                        </div>
                    </dl>
                </div>
                                                        ...
                                                        <table className="w-full border mt-2">
                                                            <thead>
                                                                <tr className="bg-gray-200">
                                                                    ...
                                                                    <th className="border p-2 w-fit">Remarks</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td className="w-fit border text-center">
                                                                            <button
                                                                                ...
                                                                                onClick={() => { openRemarksModal(catIndex, kraIndex, kpiIndex, false, false); }}
                                                                            >
                                                                                Remarks
                                                                            </button>
...
export default ViewParticularFormByID;
